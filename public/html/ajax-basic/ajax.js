...
// 2. 如果需要自定义请求头，对象提供 header 或 headers 方法来设置头
// 3. 如果需要做一些前置处理操作，提供一个 before 方法，参数为要前置执行的 function
// 4. 请求方法名决定使用的是哪种 method 提交请求，支持 get, post，可扩展,此类方法执行后，请求被发起
// 5. 提供 success 方法用来绑定 readystate==4 && status==200 时的回调处理函数，回调函数参数为 responseText 或 JSON
//    success 时试图将返回结果转json，如果成功，参数为转换后的 json 对象，否则为 string
